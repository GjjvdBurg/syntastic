...
if exists('g:loaded_syntastic_python_ruff_checker')
    finish
endif
let g:loaded_syntastic_python_ruff_checker = 1

let s:save_cpo = &cpo
set cpo&vim

function! SyntaxCheckers_python_ruff_GetHighlightRegex(item)
    return SyntaxCheckers_python_pyflakes_GetHighlightRegex(a:item)
endfunction

function! SyntaxCheckers_python_ruff_GetLocList() dict
    ...
    for e in loclist
        " ruff codes: https://beta.ruff.rs/docs/rules/
